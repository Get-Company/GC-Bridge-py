...
class APIClient:
    ...
    def set_customer_addresses_orders_and_credentials(self, customer: SW5CustomerEntity):
        ...
        duplicate_customers = self.get_customer_filter_by_email_and_not_like_id(customer.get_webshopid(), customer.get_email())
    ...
    def set_customer_adrnr_by_Id(self, customerId, adrnr):
        data = {
            'number': adrnr  # SW5 Api calls it number, db calls it customernumber and backend calls it Kundennummer.Momahdu, NaPataMahDu, JadaPataMahda, NaMahddaMoa
        }
